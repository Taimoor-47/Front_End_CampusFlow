...
| Active implementation | **P0-04, P0-05, and P0-10 are complete; P0-07 is implemented.** Passwords use PBKDF2 with legacy upgrade-on-login; emails are normalized and uniquely indexed per role with safe 409s; login/register are rate-limited per IP (10/min login, 5/min register) returning ProblemDetails 429s; auth cookie/token lifetimes are configuration-driven; both dashboards verify the session server-side (`/api/student/me`, `/api/teacher/me`) and redirect to login on 401. |
| Next recommended task | Begin P0-08 in bounded chunks: authorization-checked file downloads replacing public static serving of `/uploads/**`. |
...
| Known immediate blocker | Uploaded files remain publicly downloadable via static files (P0-08). Account lockout/backoff and full CSRF antiforgery tokens are deliberately deferred from P0-07 and recorded as follow-ups. A JWT signing key remains committed in `appsettings.json` and must be rotated before any deployment. |
