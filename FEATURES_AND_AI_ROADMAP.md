...
| Active implementation | **JWT signing-key rotation is implemented, verified, and awaiting review as an uncommitted diff.** The development key is stored in .NET User Secrets; tracked configuration contains only a placeholder. CI workflows are active in both repositories, and their first GitHub runs passed. |
| Next recommended task | Review and commit the JWT-rotation diff; push only after explicit approval. Then inspect the P0-03 DTO tail (`my-gpa`, `my-schedules`, `add-gpa`, and `add-schedule`) as the next single feature. |
| Latest product decision | Secrets stay outside source control: .NET User Secrets for local development and environment variable `Jwt__Key` for production. Rotating the signing key invalidates existing JWTs, so users must sign in again. |
| Latest verified code change | The exposed JWT signing key was replaced in tracked `appsettings.json` by a short placeholder, and a new cryptographically random 64-character Base64 key was installed in .NET User Secrets. `README.md` now documents local and production secret sources. |
| Verification baseline | Backend forced Release rebuild: 0 errors / 37 existing nullable warnings. Backend xUnit suite: **32/32 passing**. The rotated key was also live-verified through login, cookie issuance, and an authenticated endpoint. Both GitHub CI workflows passed their first runs. |
| Known immediate blocker | The JWT-rotation changes are still uncommitted, so the current remote tip and Git history still contain the old compromised value. Production must provide `Jwt__Key`; User Secrets configure only this development machine. Account lockout/backoff, full CSRF antiforgery, malware scanning, and object storage remain deferred follow-ups. |
...
| Current-student identity | Yes | Yes | **Code-aligned** | `GET /api/student/me` (and the new `GET /api/teacher/me`) are called by both dashboards on mount to verify the cookie-backed session before rendering; a 401 clears stale `sessionStorage` and redirects to login. |
...
| Account lockout/rate limits | Partial | Yes | **Partial** | Login and register endpoints are rate-limited per IP (fixed window: 10/min login, 5/min register; ProblemDetails 429). Account lockout/backoff is deliberately deferred (needs schema changes and DoS-by-lockout product decisions). |
...
| JWT cookie authentication | **Implemented** | HTTP-only, secure, role-bearing JWT cookies use configurable lifetimes. The signing key is outside source control (User Secrets locally; `Jwt__Key` in production). The 2026-08-26 rotation invalidated previously issued tokens. CSRF/session strategy still needs a production review. |
...
| Automated tests | **Partial** | The backend has 32 passing xUnit service/unit tests. Frontend unit tests, API integration/contract tests, and browser end-to-end tests are still missing. |
| CI/CD | **Implemented and first-run green** | Backend `.github/workflows/dotnet.yml`: restore → Release build → test → EF pending-model-changes gate (targets .csproj directly because .NET 8 SDK cannot parse .slnx). Frontend `.github/workflows/frontend.yml`: npm ci → tsc → blocking eslint → production build. First GitHub Actions runs on 2026-08-26 both succeeded (backend run 32972490948, frontend run 32972472807) with no YAML adjustments needed. Contract tests and browser E2E remain future additions to the pipelines. Pre-existing npm audit findings surfaced during setup: 9 (1 critical) — triage by reachability required before any deployment. |
...
| `GET` | `/api/student/me` | Student | Used to verify the dashboard session on mount |
...
| `GET` | `/api/teacher/me` | Teacher | Used to verify the dashboard session on mount |
...
| P0-07 | Harden authentication | **Partially complete (2026-08-26):** per-IP fixed-window rate limits on all four public auth endpoints (`auth-login` 10/min, `auth-register` 5/min) with ProblemDetails 429 responses, verified by live burst test (401×10 → 429×2 on login; 200/409×4 → 429×2 on register). Cookie SameSite/lifetime and JWT token lifetime come from configuration (`AuthCookie:` section, `Jwt:TokenLifetimeDays`); cookie writing is centralized in `AuthCookieService`. Dashboards verify sessions via `/api/student/me` and `/api/teacher/me`, redirecting to login on 401. The formerly committed signing key was rotated: development now uses a 64-character Base64 key in .NET User Secrets, tracked configuration contains only a placeholder, and production must provide `Jwt__Key`. **Deferred follow-ups:** account lockout/backoff (schema + product decisions), full CSRF antiforgery tokens (deployment-topology decision), forwarded-header-aware client IPs behind reverse proxies. |
| P0-08 | Harden uploads | **Partially complete (2026-08-26):** downloads are now authorization-checked. New `IFileDownloadService`/`FileDownloadService` enforces brief access = owning section teacher OR actively enrolled student; submission access = submitting student OR section teacher; unknown and forbidden files are indistinguishable (null → 404). `FilesController` exposes `GET /api/files/assignments/{name}` and `/api/files/submissions/{name}` with JWT identity, allowlist-based content types, attachment disposition with server-generated names, and path-traversal guards; `UseStaticFiles` removed so `/uploads/**` no longer resolves publicly. Frontend `getApiFileUrl` remaps upload paths to the API endpoints so all existing links work unchanged. Verified by 9 new unit tests (32 total), live checks (files endpoint 401 without cookie; old static path 404), build/test parity. **Deferred follow-ups:** malware scanning, file-signature/MIME sniffing beyond extensions, object storage migration, retention rules — deployment-stage work. |
...
| P0-11 | Add automated quality gates | **Partially complete (2026-08-26):** CI pipelines are active in both repos. Backend: restore, Release build, xUnit suite (32 tests), and `dotnet ef migrations has-pending-model-changes` as a drift gate. Frontend: npm ci, typecheck, blocking lint, production build. Both first GitHub Actions runs passed without YAML corrections. **Remaining for full completion:** API contract/integration tests in the pipeline and a small browser E2E suite. |
...
| 2026-08-26 | P0-07 authentication hardening (bounded scope) | **Code-aligned + live-verified** | Status: Implemented in approved chunks 1–3; lockout and antiforgery tokens deliberately deferred. Product decision: rate limiting chosen as the brute-force control for now; lockout deferred due to schema impact and DoS-by-lockout risk; SameSite stays configurable with dev default `None` (cross-origin localhost flow), production should tighten via config once topology is fixed (§12 item 10). Backend changes: new `AuthCookieOptions`, `IAuthCookieService`/`AuthCookieService` (single cookie writer, name `jwt`); `JwtServices` reads `Jwt:TokenLifetimeDays`; controllers use the shared cookie service; `[EnableRateLimiting]` on all four public auth endpoints with per-IP fixed windows (`auth-login` 10/min, `auth-register` 5/min) and ProblemDetails 429 via `OnRejected`; new `GET /api/teacher/me`. Frontend changes: `getCurrentStudent()`/`getCurrentTeacher()` services; both dashboards verify session server-side before loading data, clearing stale sessionStorage and redirecting to `/LoginPage` on 401. Database changes: none. Tests and verification: suite GREEN 23/23; Release build 0 errors / 37 warnings (parity); `npx tsc --noEmit` and `npm run build` pass; live burst test against a running instance confirmed login 401×10 → 429×2, register 200/409×4 → 429×2, protected route still 401 without cookie, 429 body is safe camelCase ProblemDetails. Test artifact noted: teacher account `b@t.dev`/`password123` was created in the local dev DB by the register burst — delete or ignore locally. Known limitations: rate limiter partitions by `RemoteIpAddress` (naive behind reverse proxies); no automated middleware tests (P0-11); CSRF antiforgery not yet implemented. Next action: user review of both diffs; then P0-08 protected downloads. |
| 2026-08-26 | P0-08 protected file downloads | **Code-aligned + live-verified** | Status: Implemented in approved chunks A–C. Product decision: brief visibility follows the existing enrollment rule; submissions are strictly owner-student + section-teacher; malware/signature/object-storage deferred to deployment stage. Backend changes: new `IFileDownloadService`/`FileDownloadService` (authorization resolution via AppDbContext projections); new `FilesController` (`[Authorize]`, claim-derived identity, extension→content-type map, `PhysicalFile` with attachment disposition, traversal guards); `UseStaticFiles` removed from the pipeline; DI registration added. Frontend changes: `getApiFileUrl` now maps `/uploads/{kind}/{file}` to `${API}/files/{kind}/{file}` — all brief/submission links update automatically. Database changes: none. Tests and verification: 9 new InMemory tests covering enrolled/not-enrolled students, owning/foreign teachers, owner/other students, unknown files (suite GREEN 32/32); Release build 0 errors / 37 warnings (parity); frontend `tsc --noEmit` + `next build` pass; live instance check confirmed `/api/files/*` returns 401 without cookie and `/uploads/**` returns 404. Known limitations: authorization tests are service-level only (no integration test through the controller yet, P0-11); existing stored paths must keep the exact `/uploads/{kind}/{file}` shape (FileStorageService guarantees this). Next action: user review of both diffs; then P0-11 CI quality gates. |
| 2026-08-26 | P0-11 CI quality gates | **Code-aligned (first run pending push)** | Status: Implemented per approved chunks 1–4. Product decision: lint is a blocking gate now that findings are zero; contract/E2E suites remain future pipeline additions. Frontend changes: fixed pre-existing lint debt — escaped apostrophe in `ScheduleTimeline.tsx`; removed unused `Download` import in `Grades/page.tsx`; wired the previously-dead grade-card error banner via `setError` in the mock-fallback catch; suppressed `set-state-in-effect` on the sessionStorage hydration call using the same pattern as the dashboards; added `.github/workflows/frontend.yml` (npm ci → tsc → lint → build). Backend changes: added `.github/workflows/dotnet.yml` (restore → Release build → test → EF pending-model-changes gate) targeting `.csproj` files directly because the .NET 8 SDK cannot parse `.slnx`. Database changes: none. Tests and verification: every workflow command executed locally and green — backend 37 warnings / 0 errors + 32/32 tests + no pending model changes; frontend `npm ci`, `tsc` exit 0, lint clean, build compiled successfully. Known limitations: workflows unproven until pushed to GitHub (first run may need YAML adjustments); `npm ci` surfaced 9 audit findings (1 critical) requiring reachability triage before deployment; transient local `npm ci` failure was retried clean. Next action: user review of diffs, commit, then push both repos to trigger the first CI runs; afterwards P0-12 or deferred follow-ups. |
| 2026-08-26 | P0-11 first GitHub CI runs | **Verified** | Backend run `32972490948` and frontend run `32972472807` both passed on their first execution with no YAML changes. | Add API integration/contract tests and browser E2E coverage in later approved slices. |
| 2026-08-26 | JWT signing-key rotation | **Code-aligned + live-verified; awaiting review** | A new 64-character Base64 key is stored in .NET User Secrets without being printed or committed. Tracked `appsettings.json` now contains only a placeholder; `README.md` documents User Secrets and production `Jwt__Key`. Forced Release rebuild passed with 0 errors / 37 existing warnings; xUnit passed 32/32. The OpenCode session also verified login 200 → JWT cookie → protected endpoint 200 with the rotated key. No frontend, API, DTO, database, migration, or authorization-contract changes were required. | Review the uncommitted backend config/docs and this roadmap update. After commit, the old value remains in Git history but is invalid; production still needs its own `Jwt__Key`. Next feature: inspect the P0-03 DTO tail. |
