...
| Active implementation | **P0-04, P0-05, P0-07, and P0-10 are complete; P0-08 is implemented.** Uploaded assignment briefs and student submissions are no longer publicly served: `UseStaticFiles` is removed and downloads flow through authorization-checked `GET /api/files/{assignments|submissions}/{name}`, with section/enrollment/ownership rules enforced from JWT identity. |
| Next recommended task | Begin P0-11 in bounded chunks: stand up CI running frontend lint/typecheck/build, backend build + tests, and migration checks; then add API-layer integration tests (registration, login, roles, assignments). |
