...
| Active implementation | **P0-04 is implemented and unit-tested.** Registration and login for students and teachers use `PasswordService` (PBKDF2 via ASP.NET Core Identity `PasswordHasher`); legacy unsalted SHA-256 hashes are transparently upgraded to PBKDF2 on successful login. A new `CampusFlow.Tests` xUnit project covers hashing, login, and upgrade behavior (17 tests). |
| Next recommended task | Begin P0-05 in bounded chunks: normalize email addresses, add unique indexes on student and teacher emails, and return safe `409 Conflict` responses for duplicate registration. |
| Latest product decision | Assignments will belong to a course section/subject, not an individual student. Only actively enrolled students may list, open, download, or submit them. |
| Latest verified code change | Password hashing switched to salted PBKDF2 with versioned legacy-hash detection and rehash-on-login persistence; `PasswordHelper` deleted; `ITeacherService.Login` annotated as returning `Task<Teacher?>`. |
| Verification baseline | Frontend `npx tsc --noEmit` passed (frontend unchanged by P0-04). Backend Release build passes with 0 errors and 37 nullable warnings (down from 38). New backend test suite passes 17/17 via `dotnet test`. Migration `20260822103109_AddCourseSectionsAndEnrollments` is applied and EF reports no pending model changes (rechecked 2026-08-26). |
| Known immediate blocker | Student and teacher emails have no unique constraint and no normalization, so duplicate accounts can exist; registration does not detect duplicates safely (P0-05). Login/registration endpoints remain unthrottled (P0-07) and uploaded files are publicly downloadable (P0-08). |
...
| P0-05 | Enforce account uniqueness | Normalize email addresses and add unique indexes for student and teacher emails. Return safe `409 Conflict` responses for duplicates. Decide whether one email may exist in both role tables. |
...
| P0-10 | Remove stale client services | Remove or consolidate `loginstudentservice.ts`, `registerstudentservice.ts`, and duplicate logout utilities that reference old endpoint names. Keep one typed service contract. |
