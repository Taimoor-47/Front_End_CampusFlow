...
| Active implementation | **P0-04 through P0-08 and P0-10 are complete; P0-11 is implemented.** CI workflows exist in both repos (backend: Release build, 32-test suite, pending-model-changes gate; frontend: npm ci, typecheck, clean blocking lint, production build). Uploaded files download only through authorization-checked endpoints. |
| Next recommended task | Push both repos so the workflows execute for real, fix any first-run YAML issues, then begin P0-12 (operational basics) or the deferred follow-ups (lockout, antiforgery, key rotation). |
...
| Known immediate blocker | No CI pipeline exists (P0-11), so quality gates rely on manual runs. Account lockout/backoff and full CSRF antiforgery tokens remain deferred follow-ups. A JWT signing key remains committed in `appsettings.json` and must be rotated before any deployment. Malware scanning and object storage remain out of scope for local development. |
...
| CI/CD | **Implemented (awaiting first push run)** | Backend `.github/workflows/dotnet.yml`: restore → Release build → test → EF pending-model-changes gate (targets .csproj directly because .NET 8 SDK cannot parse .slnx). Frontend `.github/workflows/frontend.yml`: npm ci → tsc → blocking eslint → production build. Every step proven green locally on 2026-08-26; contract tests and browser E2E remain future additions to the pipelines. Pre-existing npm audit findings surfaced during setup: 9 (1 critical) — triage by reachability required before any deployment. |
...
| P0-11 | Add automated quality gates | **Partially complete (2026-08-26):** CI pipelines created in both repos. Backend: restore, Release build, xUnit suite (32 tests), and `dotnet ef migrations has-pending-model-changes` as a drift gate. Frontend: npm ci, typecheck, blocking lint, production build. Pre-existing lint debt cleared first so lint could be blocking from day one (`ScheduleTimeline` apostrophe escape; `Grades` unused import removed, unused error state wired to the demo-data banner, setState-in-effect suppressed with the established pattern). All steps verified locally; first real execution happens on next push. **Remaining for full completion:** API contract/integration tests in the pipeline and a small browser E2E suite. |
